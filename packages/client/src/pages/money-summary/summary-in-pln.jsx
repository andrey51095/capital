...
export default function SummaryInPln({ summary }) {
  const urls = useMemo(() => summary.map(({ currency }) => currency).filter(c => c !== PLN).map(getUrl), [summary]);
  const { data, loading } = useFetchAll(urls, requestOptions)

  const preparedData = data.reduce((acc, item) => ({ [item.code]: item.rates[0], ...acc }), {})
      ...
      {!loading && (
        ...
        <Block display="flex" >
          ...
          <Block paddingLeft="scale200">
            {Object.keys(preparedData).map((key) => (<Block key={ key} display="flex" gridColumnGap="scale200">
              <Currency value={key} />
